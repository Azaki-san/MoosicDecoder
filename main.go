package main

import (
	"errors"
	"flag"
	"io"
	"log"
	"os"
	"path/filepath"
	"strings"

	"github.com/bogem/id3v2"
)

var KEY = [256]byte{
	,
}

func xorBlock(block []byte) []byte {
	result := make([]byte, len(block))
	for i := 0; i < len(block); i++ {
		result[i] = block[i] ^ KEY[i%len(KEY)]
	}
	return result
}

func decodeFile(inputPath, outputPath string) error {
	inFile, err := os.Open(inputPath)
	if err != nil {
		return err
	}
	defer inFile.Close()

	outFile, err := os.Create(outputPath)
	if err != nil {
		return err
	}
	defer outFile.Close()

	buffer := make([]byte, 256)
	for {
		n, err := inFile.Read(buffer)
		if err != nil && !errors.Is(err, io.EOF) {
			return err
		}
		if n == 0 {
			break
		}
		decoded := xorBlock(buffer[:n])
		_, err = outFile.Write(decoded)
		if err != nil {
			return err
		}
	}
	return nil
}

func decodeAll(inputDir, outputDir string) error {
	entries, err := os.ReadDir(inputDir)
	if err != nil {
		return err
	}

	for _, entry := range entries {
		if entry.IsDir() || !strings.HasSuffix(entry.Name(), ".encoded") {
			continue
		}

		inputPath := filepath.Join(inputDir, entry.Name())
		tempOutput := filepath.Join(outputDir, strings.TrimSuffix(entry.Name(), ".encoded")+".mp3")

		if err := decodeFile(inputPath, tempOutput); err != nil {
			log.Printf("[×] Ошибка при обработке %s: %v", entry.Name(), err)
			continue
		}

		newName, err := extractNameFromTags(tempOutput)
		if err != nil {
			log.Printf("[!] Ошибка чтения тегов для %s: %v", entry.Name(), err)
			continue
		}

		finalPath := tempOutput
		if newName != "" {
			finalPath = filepath.Join(outputDir, newName)
			if err := os.Rename(tempOutput, finalPath); err != nil {
				log.Printf("[×] Не удалось переименовать %s: %v", tempOutput, err)
				continue
			}
		}

		log.Printf("[✓] %s -> %s", entry.Name(), filepath.Base(finalPath))
	}

	log.Println("✔ Декодирование завершено")
	return nil
}

func extractNameFromTags(mp3Path string) (string, error) {
	tag, err := id3v2.Open(mp3Path, id3v2.Options{Parse: true})
	if err != nil {
		return "", err
	}
	defer tag.Close()

	artist := strings.TrimSpace(tag.Artist())
	title := strings.TrimSpace(tag.Title())

	if artist == "" || title == "" {
		return "", nil
	}

	filename := artist + " - " + title + ".mp3"
	return filename, nil
}

func main() {
	input := flag.String("input", "", "Путь к зашифрованной папке")
	output := flag.String("output", "", "Путь к выходной папке")
	flag.Parse()

	if *input == "" || *output == "" {
		log.Fatal("Укажите --input и --output")
	}

	if err := os.MkdirAll(*output, 0755); err != nil {
		log.Fatalf("Не удалось создать выходную папку: %v", err)
	}

	err := decodeAll(*input, *output)
	if err != nil {
		log.Fatalf("Ошибка: %v", err)
	}
}
